...
func (r *InfoPostgres) GetPlanBySupID(supID string) (*Rates, error) {
	...
	query := fmt.Sprintf("SELECT SUM(p.internet), SUM(p.tv), SUM(p.convergent), SUM(p.cctv) FROM %s p INNER JOIN %s u ON p.supervisor_id = u.id AND u.is_valid = true WHERE p.supervisor_id = $1 AND DATE_TRUNC('month', p.date_time) = DATE_TRUNC('month', CURRENT_DATE)", plansTable, usersTable)
	row := r.db.QueryRow(query, supID)
	if err := row.Scan(&plan.Internet, &plan.TV, &plan.Convergent, &plan.CCTV); err != nil {
		return nil, err
	}
